...
var _WeakSet = unwrapExports(weakSet$1);
...
var observe = function observe(element, callback) {
  ...
  if (!wm) wm = new _WeakMap();
  var callbacks = wm.get(element);
  //! 已经注册过
  if (callbacks) {
    callbacks.add(callback);
  } else {
    var wkset = new _WeakSet();
    wkset.add(callback);
    wm.set(element, wkset);
  }
  console.log(wm);
};

//! 监听
var monitor = window.ResizeObserver ? observe : null;
